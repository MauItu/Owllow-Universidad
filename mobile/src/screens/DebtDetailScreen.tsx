...
export function DebtDetailScreen() {
  ...
  // Perdonar: borra el seguimiento de la deuda/préstamo sin revertir el
  // desembolso ni los abonos ya registrados (esos movimientos y su efecto en
  // el saldo se mantienen tal cual quedaron).
  const forgiveDebt = () => {
    Alert.alert(
      isDebt ? 'Perdonar deuda' : 'Perdonar préstamo',
      isDebt
        ? 'Se dejará de hacer seguimiento a esta deuda. El ingreso que recibiste al adquirirla y los pagos ya hechos NO se revertirán.'
        : 'Se dejará de hacer seguimiento a este préstamo. El gasto que registraste al prestarlo y los abonos ya recibidos NO se revertirán.',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Perdonar',
          ...
          onPress: async () => {
            try {
              await debtsApi.forgive(debtId);
              cancelDebtNotifications(debtId).catch(() => {});
              showSuccess(isDebt ? 'Deuda perdonada' : 'Préstamo perdonado');
              triggerRefresh();
              navigation.goBack();
            } catch (err) {
              showError(getErrorMessage(err));
        ...
        right={
          debt ? (
            <View style={{ flexDirection: 'row', gap: theme.spacing.sm }}>
              ...
              {!debt.isPaidOff && (
                <Pressable hitSlop={8} onPress={forgiveDebt}>
                  <Icon name="heart-handshake" size={20} color={theme.colors.onHeader} />
                </Pressable>
              )}
