...
export function AddSavingsGoalScreen() {
  ...
  // true = meta sin límite (ahorro libre, sin monto objetivo).
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setName(g.name);
        setUnlimited(g.targetAmount == null);
        setTargetAmount(g.targetAmount != null ? Number(g.targetAmount) : 0);
  ...
  const save = async () => {
    ...
    if (!unlimited && targetAmount <= 0) {
      showError('Define el monto objetivo');
      return;
    }
    const payload = {
      name: name.trim(),
      targetAmount: unlimited ? null : targetAmount,
      deadline,
      color,
      icon,
      accountId,
      ...
      <FormScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.switchRow}>
          <View style={{ flex: 1, paddingRight: theme.spacing.md }}>
            <Text style={styles.switchTitle}>Meta sin límite</Text>
            <Text style={styles.switchHint}>Ahorro libre, sin un monto objetivo que alcanzar.</Text>
          </View>
          <Switch
            value={unlimited}
