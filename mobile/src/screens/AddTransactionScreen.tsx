...
export function AddTransactionScreen() {
  ...
  // FKs de la plantilla: se resuelven a objetos en los efectos de abajo cuando
  // las listas de cuentas/categorías terminen de cargar.
  const templateAccountId = params.template?.accountId ?? null;
  const templateCategoryId = params.template?.categoryId ?? null;
  ...
  // Cuenta por defecto (solo al crear): la de la plantilla si trae una, si no
  // la primera disponible. Al editar, la cuenta la fija la transacción cargada
  // (ver efecto de resolución de FKs).
  useEffect(() => {
    if (editingId || account || accounts.length === 0) return;
    if (templateAccountId != null) {
      const fromTemplate = accounts.find((a) => a.id === templateAccountId);
      if (fromTemplate) {
        setAccount(fromTemplate);
        return;
      }
    }
    setAccount(accounts[0]);
  }, [accounts, account, editingId, templateAccountId]);

  // Categoría de la plantilla (solo al crear): se resuelve cuando las
  // categorías terminen de cargar.
  useEffect(() => {
    if (editingId || category || categories.length === 0 || templateCategoryId == null) return;
    const fromTemplate = findCategoryById(categories, templateCategoryId);
    if (fromTemplate) setCategory(fromTemplate);
  }, [categories, category, editingId, templateCategoryId]);
