...
function SavingsGoalCardComponent({ goal, onPress }: Props) {
  ...
  const target = hasTarget ? Number(goal.targetAmount) : 0;
  const progress = hasTarget && target > 0 ? Math.min(1, current / target) : 0;
    ...
    >
      <View style={styles.header}>
        ...
        {goal.isCompleted ? (
          ...
          <Text style={styles.unlimited}>Libre</Text>
        )}
      </View>

      {/* Barra de progreso con gradiente (solo con meta: sin objetivo no hay % que mostrar) */}
      {hasTarget && (
      ...
      <View style={styles.amounts}>
        ...
        {hasTarget && <Text style={styles.target}>de {formatCurrency(target)}</Text>}
      </View>
    </Pressable>
  );
}
...
const createStyles = (theme: Theme) =>
  StyleSheet.create({
    ...
    unlimited: { color: theme.colors.textMuted, fontSize: theme.fontSize.xs, fontWeight: theme.fontWeight.semibold },
    checkBadge: {
      width: 28,
      height: 28,
