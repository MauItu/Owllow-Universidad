...
// POST /api/debts/:id/forgive — perdona la deuda/préstamo: borra el registro de
// seguimiento SIN revertir ni tocar las transacciones ya generadas (desembolso
// inicial y abonos). El gasto/ingreso que ya movió el saldo se mantiene tal
// cual: perdonar no descuenta ni agrega nada a las cuentas.
debtsRouter.post(
  '/:id/forgive',
  asyncHandler(async (req, res) => {
    const uid = userId(req);
    ...
    // CASCADE borra debt_payments; las transacciones enlazadas (initialTransactionId
    // y las de cada abono) no se tocan, así que su efecto en el saldo persiste.
    await db.delete(debts).where(and(eq(debts.id, id), eq(debts.userId, uid)));
    res.json({ success: true });
  }),
);
