...
const goalSchema = z.object({
  ...
  // null/ausente = meta sin límite (ahorro libre).
  targetAmount: z.union([z.coerce.number().positive(), z.null()]).optional(),
  deadline: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/)
    .optional()
  ...
  asyncHandler(async (req, res) => {
    const [row] = await db
      .select({
        ...
        // Por meta (no por agregado): así una meta sin límite (target NULL) no
        // "contamina" el restante de las demás metas con objetivo.
        totalRemaining: sql<string>`COALESCE(SUM(GREATEST(${savingsGoals.targetAmount} - ${savingsGoals.currentAmount}, 0)), 0)`,
    ...
    res.json({
      totalSaved: Number(row.totalSaved),
      totalTarget: Number(row.totalTarget),
  ...
  asyncHandler(async (req, res) => {
    ...
    const [row] = await db
      ...
      .values({
        userId: userId(req),
        name: data.name,
        targetAmount: data.targetAmount != null ? data.targetAmount.toFixed(2) : null,
...
savingsRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Si cambia el objetivo (incluido quitarlo → meta sin límite), recalcular el
    // estado de completitud. target == null → nunca se marca completada.
    const target = data.targetAmount !== undefined ? data.targetAmount : old.targetAmount != null ? Number(old.targetAmount) : null;
    const completedNow = target != null && Number(old.currentAmount) >= target;
  ...
  asyncHandler(async (req, res) => {
    ...
    const [updated] = await db
      ...
      .set({
        ...
        // COALESCE(..., false): con meta sin límite (target NULL) la comparación
        // da NULL y nunca se marca completada.
        isCompleted: sql`COALESCE((${newAmountExpr}) >= ${savingsGoals.targetAmount}, false)`,
...
savingsRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const completed = goal.targetAmount != null && rounded >= Number(goal.targetAmount);
